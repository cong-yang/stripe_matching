function [match_cost] = f_innerdist(im1, im2, mat, n_contsamp)
...
for k=1:2
    scs{k}	= mat{2,floor(find(strcmp(mat,sIms{k}))/2)+1};
    
% 	if exist(strcat('temp/',sIms{k},'.mat'))~=2
%         %- Contour extraction
...
% 									0);
%         save(strcat('temp/',sIms{k},'.mat'),'sc');
%     else
%         load(strcat('temp/',sIms{k},'.mat'));
%     end
%     scs{k}	= sc;
